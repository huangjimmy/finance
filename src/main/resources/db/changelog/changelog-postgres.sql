...
CREATE TABLE StockSymbol (id BIGINT GENERATED ALWAYS AS IDENTITY, symbol VARCHAR(30), name VARCHAR(255), exchange VARCHAR(10), PRIMARY KEY(id), UNIQUE(symbol, exchange))
-- changeset liquibase:3
ALTER TABLE StockSymbol RENAME TO Stock_Symbol
-- changeset liquibase:4
CREATE TABLE Stock_Historical_Price(id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
trading_day DATE,
...
CREATE TABLE Stock_Dividends(id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
 exchange VARCHAR(10), symbol VARCHAR(30),
 trading_day DATE,
 declaration_date DATE, ex_dividend_date DATE, record_date DATE, payment_date DATE, dividend_per_share numeric(10, 4),
UNIQUE(trading_day, exchange, symbol));

 CREATE TABLE Stock_Dividend_Splits(id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
  exchange VARCHAR(10), symbol VARCHAR(30),
  trading_day DATE,
...
CREATE TABLE yfinance_cache (ticker VARCHAR(60) PRIMARY KEY, history BYTEA, splits BYTEA, dividends BYTEA, expiresAt timestamp);